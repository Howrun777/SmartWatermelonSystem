...
国外在近红外光谱无损检测技术方面的研究起步较早，技术相对成熟。欧美发达国家自20世纪90年代起就开始将近红外光谱技术应用于水果内部品质检测。美国NASA早在21世纪初就开展了利用近红外光谱技术检测水果糖度的研究，并在ISS国际空间站的果蔬储存实验中验证了相关技术的可行性。日本作为农业发达国家，其农林水产省下属研究机构长期致力于近红外光谱技术在水果检测领域的研究，已开发出多款商用便携式水果糖度检测仪。

近年来，随着物联网技术的发展，嵌入式光谱检测设备逐渐成为研究热点。荷兰瓦赫宁根大学的研究团队开发了基于微型光谱传感器的农产品品质检测系统；以色列农业研究组织（ARO）也在西瓜糖度近红外检测方面进行了系统研究，建立了针对不同西瓜品种的糖度预测模型。这些研究为本系统设计提供了重要的理论基础和技术参考。

#### 1.2.2 国内研究现状

国内在果蔬近红外无损检测领域的研究起步相对较晚，但近年来发展迅速。中国农业大学、华中农业大学、浙江大学等高校在水果内部品质近红外检测方面开展了大量研究工作，在特征波长选择、建模算法优化、检测精度提升等方面取得了丰硕成果。

在嵌入式便携检测设备方面，国内研究主要集中在基于STM32单片机的水果糖度检测系统开发，以及基于Android/iOS平台的手机光谱检测应用。然而，现有研究多停留在实验室阶段，存在以下不足：（1）设备成本控制困难，商用化程度低；（2）缺乏完善的离线数据存储机制；（3）未考虑消费端和生产端的不同使用场景需求；（4）传感器缺失时的容错处理机制不完善。
...
本系统采用经典的三层物联网架构设计，从下到上依次为：终端感知层、数据服务层、应用展示层。
...
本系统的糖度检测基于朗伯-比尔定律（Lambert-Beer Law）。朗伯-比尔定律是光吸收的基本定律，描述了光被介质吸收的程度与介质厚度及浓度之间的关系。含氢基团（如O-H、C-H）在近红外光谱区（780nm~2526nm）有特征的吸收峰，西瓜中的糖分（主要成分为蔗糖、葡萄糖、果糖）含有大量O-H键，因此可以通过检测特定波长处的光吸收强度来推算糖度。

AS7341光谱传感器提供11个光谱通道，覆盖了糖度检测的关键波段。本系统选取ch555（555nm）、ch640（640nm）、ch680（680nm）和chNIR四个通道进行多元线性回归（MLR）建模，通过吸收率计算间接得到糖度值。

具体算法实现如下：

```
abs_555 = log10(chClear / (ch555 + 1))
abs_640 = log10(chClear / (ch640 + 1))
abs_680 = log10(chClear / (ch680 + 1))
abs_NIR = log10(chClear / (chNIR + 1))

raw_brix = (-1.2) × abs_555 + 3.5 × abs_640 + 6.8 × abs_680 + 12.5 × abs_NIR - 2.5
```

系数通过前期实验数据拟合获得，设备端与后端使用完全相同的算法实现，确保检测结果的一致性。
...
[5] Esposito A, Crimaldi M, Sarghini F. Application of near infrared spectroscopy for the real-time detection of fruit quality: A review [J]. Sensors, 2022, 22(3): 892.

[6] Liu Y D, Sun X F, Zhang H G. Rapid and non-destructive determination of sugar content in watermelon using visible/near-infrared spectroscopy [J]. Journal of Food Measurement and Characterization, 2019, 13(4): 2745-2753.

[7] Nicolai B M, Beullens K, Bobelyn E, et al. Nondestructive measurement of fruit and vegetable quality by means of NIR spectroscopy: A review [J]. Postharvest Biology and Technology, 2007, 46(2): 99-118.

[8] Esquivel P, Jimenez V M. Functional compounds in bell pepper: Antioxidants, vitamin C and vitamin A [M]. Springer, 2018.

[9] Xu L, Yan S M, Shi C Y. Fruit quality detection system based on machine vision and near-infrared spectroscopy [J]. Journal of Agricultural Science, 2018, 10(12): 231-242.

[10] Huang H, Yu S J, Xu H Y. Design of wireless sensor network node for agricultural environment monitoring [J]. IFAC Proceedings Volumes, 2013, 46(4): 256-261.

[11]乐鑫科技. ESP32技术参考手册 [EB/OL]. [https://www.espressif.com/](https://www.espressif.com/), 2023.

[12] LVGL. Light and Versatile Graphics Library Documentation [EB/OL]. [https://docs.lvgl.io/](https://docs.lvgl.io/), 2024.

[13] Myers H G, Pylvänäinen J W, Sutton D J. LittleFS: A fail-safe file system for embedded devices [C]. 2017 USENIX Annual Technical Conference, 2017: 85-98.
