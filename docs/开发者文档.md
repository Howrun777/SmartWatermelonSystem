...
│       │   ├── SugarCalc.h       # SugarCalc::calculate（基于朗伯-比尔定律吸收率模型）
│       │   ├── SugarCalc.cpp     # 公式：raw_brix = -1.2*abs_555 + 3.5*abs_640 + 6.8*abs_680 + 12.5*abs_NIR - 2.5
...
│   │
│   ├── admin/
│   │   ├── login.html            # 登录页（密码显隐、错误提示、回车提交、spinner加载）
│   │   ├── dashboard.html        # 监控大屏（西瓜阵列+4个ECharts图表+实时时钟+注销）
│   │   │
│   │   ├── css/
│   │   │   └── style.css        # 样式表：毛玻璃导航、卡片、西瓜阵列、离线告警动画、响应式
│   │   │
│   │   └── js/
│   │       ├── api.js           # BASE_URL，login()封装（POST /api/admin/login）
│   │       ├── chart.js         # ECharts初始化、buildStrictOption（严格坐标轴）、
│   │       │                       renderWatermelonGrid（掉线30分钟感知）、
│   │       │                       renderWmHistoryChart、renderEnvHistoryCharts
│   │       │                       Y轴色彩映射、Tooltip时间格式化、MarkLine阈值线
...
│  └─ 瓜田号合法性校验（匹配生产信息表）
├─ 数据计算模块
│  ├─ 光谱→糖度计算（基于朗伯-比尔定律吸收率模型：
│  │   abs_555=log10(chClear/(ch555+1)), abs_640=log10(chClear/(ch640+1))
│  │   abs_680=log10(chClear/(ch680+1)), abs_NIR=log10(chClear/(chNIR+1))
│  │   Sugar = -1.2*abs_555 + 3.5*abs_640 + 6.8*abs_680 + 12.5*abs_NIR - 2.5）
│  │   信号质量检测：chClear<200 或 chNIR<10 时返回0.0
...
│  ├─ Hero区域：产品介绍 + 技术规格面板
│  ├─ 统计栏：5个核心指标卡片
│  ├─ 功能特性：3列卡片（无损检测/环境监控/智能大屏）
│  ├─ 技术架构：四层架构图（感知/传输/处理/展示）
│  ├─ 数据流向：完整链路展示
│  └─ 响应式布局支持移动端
├─ 管理员登录模块（admin/login.html + js/api.js）
│  ├─ 品牌头部：西瓜图标 + 标题
│  ├─ 表单验证：账号/密码输入
│  ├─ 密码显隐切换
│  ├─ 错误/成功提示框
│  ├─ 加载状态spinner动画
│  ├─ 回车键快捷提交
│  └─ POST /api/admin/login → 跳转dashboard
├─ 监控大屏模块（admin/dashboard.html）
│  ├─ 顶部导航栏
│  │  ├─ 瓜田切换按钮（◀/▶）
│  │  ├─ 强制刷新按钮
│  │  ├─ 实时系统时间
...
│     ├─ 环境湿度折线图
│     └─ 光照强度折线图
├─ 图表引擎（admin/js/chart.js）
│  ├─ buildStrictOption：严格坐标轴配置
│  ├─ renderWatermelonGrid：西瓜阵列渲染
│  ├─ renderWmHistoryChart：糖度历史图
│  ├─ renderEnvHistoryCharts：环境数据图
│  ├─ X轴：type='value'纯数值轴
│  ├─ Y轴：色彩映射分区着色
│  ├─ Tooltip：毫秒时间戳转可读时间
│  └─ MarkLine：成熟度阈值线
└─ 自动切换（admin/js/auto_switch.js）
   ├─ idleTimer：60秒无操作切换瓜田
   ├─ dataRefreshTimer：每6秒静默刷新
   ├─ getTimeRange：计算4档时间范围
   ├─ reloadAllChartsData：重载所有图表数据
   ├─ forceRefresh：强制刷新入口
   └─ 监听：mousemove/mousedown/keypress
```
...
| **主键冲突解决** | 同一秒同一瓜田重复上传 → 写入失败但不影响西瓜数据入库                                              |
| **糖度计算算法** | 基于朗伯-比尔定律吸收率模型：<br>abs_555=log10(chClear/(ch555+1)), abs_640=log10(chClear/(ch640+1))<br>abs_680=log10(chClear/(ch680+1)), abs_NIR=log10(chClear/(chNIR+1))<br>Sugar = -1.2*abs_555 + 3.5*abs_640 + 6.8*abs_680 + 12.5*abs_NIR - 2.5 |
| **信号质量检测** | chClear < 200 或 chNIR < 10 时判定为测量异常，返回糖度0.0                                     |
...
- **请求方式**：`POST`
- **实现方式**：前端清除Cookie，后端依赖Session自动过期（Max-Age=3600）
- **前端实现代码**：
...
- **注意**：后端未实现主动Session销毁接口，仅通过Cookie过期机制实现登出
...
1. 管理员登录 → 验证密码(Argon2) → 获取Session Cookie
2. 进入后台(dashboard.html) → 请求瓜田列表 → 渲染分页控件
3. 静默刷新（每6秒）→ 请求当前瓜田西瓜实时数据
4. 左侧渲染西瓜阵列，右侧渲染环境图表（温度/湿度/光照）
5. 点击西瓜卡片 → 请求该西瓜历史糖度数据 → 渲染折线图
6. 60秒无操作自动切换下一个瓜田巡航
7. 点击"注销" → 清除Cookie → 跳转登录页

### 3. 关键规则落地

- **环境数据无效**：温湿度光照全为99 → 跳过field_environment写入，西瓜数据不受影响
- **主键冲突**：同一秒同一瓜田重复上传 → 西瓜数据入库，环境数据失败不影响主流程
- **成熟度计算**：`成熟度 = 计算糖度 / 瓜田成熟糖度阈值`
- **糖度算法**：`raw_brix = -1.2*abs_555 + 3.5*abs_640 + 6.8*abs_680 + 12.5*abs_NIR - 2.5`
  - 其中 `abs_xxx = log10(chClear / (chxxx + 1))`
- **信号质量检测**：chClear < 200 或 chNIR < 10 时返回糖度0.0
- **设备禁用**：`device_auth`表`status=0` → 401拒绝上传
- **登出机制**：前端清除Cookie，后端Session通过Max-Age=3600自动过期
