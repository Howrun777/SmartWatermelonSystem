...
```
// 1. 环境判据（信号质量检测）
if (ch445 < 10.0 || ch480 < 10.0 || ch680 < 10.0 || chNIR < 10.0) return 0.0;
...
// 糖度计算
Sugar = 13.21941763 - 23.50752069*X445 + 8.59055811*X480 + 5.98831600*X680 - 8.41861824*XNIR

// 物理极限约束：0.0 ~ 20.0 Brix
```

**信号质量检测**：ch445、ch480、ch680 或 chNIR < 10 时判定为测量异常，返回0.0；chClear仅用于曝光状态记录
...
// Step 2: MLR回归计算
Sugar (Brix) = 13.21941763 - 23.50752069*X445 + 8.59055811*X480 + 5.98831600*X680 - 8.41861824*XNIR
...
| ch480 | 480nm | 蓝绿光过渡通道 |
| ch680 | 680nm | 红光吸收区 |
| chClear | 全光谱 | 曝光状态记录，不参与分母归一化 |
| chNIR | 近红外 | 近红外敏感通道 |

**质量判据**：ch445、ch480、ch680 或 chNIR < 10 时返回0.0（测量异常）
